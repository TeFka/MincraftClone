...
World::World(GLFWwindow* newWindow, const Camera &camera, const Config &config, Player &player)
    : m_chunkManager(*this)
...
void World::update(const Camera &camera)
{
    ...
    if(glfwGetKey(theWindow, GLFW_KEY_C)==GLFW_PRESS) {
...
void World::updateChunk(int blockX, int blockY, int blockZ)
{
    ...
    if (sectionBlockXZ.x == 0) {
        glm::ivec3 newKey(chunkPosition.x - 1, chunkSectionY,
                            chunkPosition.z);
        addChunkToUpdateBatch(
            newKey,
    ...
    if (sectionBlockY == 0) {
        glm::ivec3 newKey(chunkPosition.x, chunkSectionY - 1,
                            chunkPosition.z);
        addChunkToUpdateBatch(
            newKey,
            m_chunkManager.getChunk(newKey.x, newKey.z).getSection(newKey.y));
    ...
    if (sectionBlockXZ.z == 0) {
        glm::ivec3 newKey(chunkPosition.x, chunkSectionY,
                            chunkPosition.z - 1);
        addChunkToUpdateBatch(
            newKey,
            m_chunkManager.getChunk(newKey.x, newKey.z).getSection(newKey.y));
...
void World::setSpawnPoint()
{
    ...
    m_playerSpawnPoint = {worldX, blockY+5, worldZ};
    ...
    std::cout << "Spawn found! Attempts: " << attempts
              << " Time Taken: " << (glfwGetTime()-previousTime)
              << " seconds\n";
}
